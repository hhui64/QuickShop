...
public class ContainerShop implements Shop {
  ...
  public void update() {
    ...
    try {
      plugin.getDB().execute(q, this.getOwner().toString(), Util.serialize(this.getItem()), unlimited, shopType.toID(),
          this.getPrice(), x, y, z, world);
    } catch (Exception e) {
      e.printStackTrace();
    ...
    if (this.isUnlimited()) {
      ...
      if (amount > 0) {
        plugin.getLogger().log(Level.WARNING, "Could not take all items from a players inventory on purchase! "
            + p.getName() + ", missing: " + amount + ", item: " + this.getDataName() + "!");
      }
    } else {
   ...
   * 
   * @param owner The name of the owner. You must do shop.update() after to save
   *              it after a reboot.
   */
   ...
   * Deletes the shop from the list of shops and queues it for database deletion
   * 
   * @param fromMemory True if you are *NOT* iterating over this currently, *false
  ...
  public String toString() {
    StringBuilder sb = new StringBuilder(
        "Shop " + (loc.getWorld() == null ? "unloaded world" : loc.getWorld().getName()) + "(" + loc.getBlockX() + ", "
