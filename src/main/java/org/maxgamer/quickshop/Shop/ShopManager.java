...
public class ShopManager {
  ...
  /**
   * @return Returns the HashMap<Player name, shopInfo>. Info contains what their
   *         last question etc was.
   */
  ...
  public void createShop(Shop shop) {
    ...
    try {
      ...
      plugin.getDB().execute(q, shop.getOwner().toString(), shop.getPrice(), Util.serialize(item), loc.getBlockX(),
  ...
  /**
   * Loads the given shop into storage. This method is used for loading data from
   * the database. Do not use this method to create a shop.
   * 
  ...
  public boolean canBuildShop(Player p, Block b, BlockFace bf) {
    RegisteredListener openInvRegisteredListener = null; // added for compatibility reasons with OpenInv - see
    ...
    try {
      ...
      PlayerInteractEvent pie = new PlayerInteractEvent(p, Action.RIGHT_CLICK_BLOCK, AIR, b, bf); // PIE =
                                                                                                  // PlayerInteractEvent
                                                                                                  // - What else?
      ...
      public void run() {
          ...
          try {
            ...
            if (priceRestriction != null) {
              if (price < priceRestriction.getKey() || price > priceRestriction.getValue()) {
                p.sendMessage(ChatColor.RED + "Restricted prices for " + info.getItem().getType() + ": min "
                    + priceRestriction.getKey() + ", max " + priceRestriction.getValue());
              }
            }
            ...
            createShop(shop);
            Location loc = shop.getLocation();
            plugin.log(p.getName() + " created a " + shop.getDataName() + " shop at (" + loc.getWorld().getName()
        ...
        /* Purchase Handling */
          ...
          if (shop.isSelling()) {
            ...
            if (stock < amount) {
              p.sendMessage(
                  MsgUtil.getMessage("shop-stock-too-low", "" + shop.getRemainingStock(), shop.getDataName()));
              return;
            }
            if (amount == 0) {
              // Dumb.
            ...
            if (!p.getUniqueId().equals(shop.getOwner())) {
              ...
              if (shop.getOwner().equals(p.getUniqueId())
                  || Permissions.hasPermission(Bukkit.getOfflinePlayer(shop.getOwner()), "quickshop.taxexemption")) {
                tax = 0;
              }
              ...
              if (plugin.getConfig().getBoolean("show-tax")) {
                String msg = MsgUtil.getMessage("player-bought-from-your-store-tax", p.getName(), "" + amount,
                    shop.getDataName(), Util.format((tax * total)));
                if (stock == amount)
                  msg += "\n" + MsgUtil.getMessage("shop-out-of-stock", "" + shop.getLocation().getBlockX(),
                      "" + shop.getLocation().getBlockY(), "" + shop.getLocation().getBlockZ(), shop.getDataName());
            ...
            MsgUtil.sendPurchaseSuccess(p, shop, amount);
            plugin.log(
                p.getName() + " bought " + amount + " for " + (shop.getPrice() * amount) + " from " + shop.toString());
              ...
              if (!shop.isUnlimited() || plugin.getConfig().getBoolean("shop.pay-unlimited-shop-owners")) {
                ...
                if (plugin.getEcon().getBalance(shop.getOwner()) < amount * shop.getPrice()) {
                  p.sendMessage(MsgUtil.getMessage("the-owner-cant-afford-to-buy-from-you",
                      format(amount * shop.getPrice()), format(plugin.getEcon().getBalance(shop.getOwner()))));
                  return;
                }
            ...
            plugin
                .log(p.getName() + " sold " + amount + " for " + (shop.getPrice() * amount) + " to " + shop.toString());
          }
  ...
  public class ShopIterator implements Iterator<Shop> {
    ...
    /**
     * Removes the current shop. This method will delete the shop from memory and
     * the database.
     */
    @Override
    public void remove() {
