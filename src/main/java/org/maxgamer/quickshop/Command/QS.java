...
public class QS implements CommandExecutor {
    ...
    if (sender instanceof Player && sender.hasPermission("quickshop.unlimited")) {
      ...
      while (bIt.hasNext()) {
        ...
        if (shop != null) {
          ...
          shop.update();
          sender.sendMessage(
              MsgUtil.getMessage("command.toggle-unlimited", (shop.isUnlimited() ? "unlimited" : "limited")));
          return;
        }
      }
  ...
  private void export(CommandSender sender, String[] args) {
    ...
    if (type.startsWith("sql") || type.contains("file")) {
      ...
      try {
        ...
        e.printStackTrace();
        sender.sendMessage(
            ChatColor.RED + "Failed to export to SQLite: " + file.toString() + " Reason: " + e.getMessage());
      }
      return;
    }
    ...
    if (args.length > 0) {
      ...
      if (subArg.equals("unlimited")) {
        ...
        if (sender.hasPermission("quickshop.info")) {
          ...
          sender.sendMessage(ChatColor.GREEN + "" + (buying + selling) + " shops in " + chunks + " chunks spread over "
              + worlds + " worlds.");
  ...
  public void sendHelp(CommandSender s) {
    ...
    if (s.hasPermission("quickshop.create.buy"))
      s.sendMessage(
          ChatColor.GREEN + "/qs buy" + ChatColor.YELLOW + " - " + MsgUtil.getMessage("command.description.buy"));
    if (s.hasPermission("quickshop.create.sell")) {
      s.sendMessage(
          ChatColor.GREEN + "/qs sell" + ChatColor.YELLOW + " - " + MsgUtil.getMessage("command.description.sell"));
    ...
    if (s.hasPermission("quickshop.create.changeprice"))
      s.sendMessage(
          ChatColor.GREEN + "/qs price" + ChatColor.YELLOW + " - " + MsgUtil.getMessage("command.description.price"));
    if (s.hasPermission("quickshop.clean"))
      s.sendMessage(
          ChatColor.GREEN + "/qs clean" + ChatColor.YELLOW + " - " + MsgUtil.getMessage("command.description.clean"));
    ...
    if (s.hasPermission("quickshop.empty"))
      s.sendMessage(
          ChatColor.GREEN + "/qs empty" + ChatColor.YELLOW + " - " + MsgUtil.getMessage("command.description.empty"));
